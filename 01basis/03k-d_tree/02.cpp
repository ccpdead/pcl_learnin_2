...
//    float resolution = 0.01f;
    ...
    std::vector<float>pointIdxNKNSquareDistance;

    std::cout<<"K nearest neighbor search at ("<<searchPoint.x
             <<"    "<<searchPoint.y
             <<"    "<<searchPoint.z
             <<") with K = "<<K<<std::endl;
    if(octree.nearestKSearch(searchPoint, K, pointIdxNKNSearch, pointIdxNKNSquareDistance) > 0)
    {
        for(size_t i = 0; i< pointIdxNKNSquareDistance.size(); ++i)
        std::cout<<"    "<<cloud->points[pointIdxNKNSearch[i]].x
                 <<"    "<<cloud->points[pointIdxNKNSearch[i]].y
                 <<"    "<<cloud->points[pointIdxNKNSearch[i]].z
                 <<"(squared distance: "<<pointIdxNKNSquareDistance[i]<<")"<<std::endl;
    }
